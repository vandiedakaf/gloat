 ...
 */
public class ConfirmContestDrawTest {
    ...
    @Tested
    private ConfirmContestDraw confirmDraw;
    ...
    public void getCallbackId() throws Exception {
        assertThat(confirmDraw.getCallbackId(), is(Callbacks.CONFIRM_DRAW.toString()));
    }
    ...
    public void runGoldenNo() throws Exception {
        ...
        callbackRequest.setCallbackId(Callbacks.CONFIRM_DRAW.toString() + "|" + USER_ID);

        Response response = confirmDraw.run(callbackRequest);
        assertThat(response.getAttachments().get(0).getText(), containsString("You've opted not to confirm this draw."));
    }
